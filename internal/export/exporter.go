...
func (e *Exporter) exportImageToPNG(img *types.Image, outputPath string) error {
	...
	if err := e.addBurntInText(grayImage, img); err != nil {
...
func (e *Exporter) addBurntInText(img *image.Gray, dicomImg *types.Image) error {
	// Create text lines for burnt-in metadata
	textLines := []string{
		fmt.Sprintf("SOP Instance UID: %s", dicomImg.SOPInstanceUID),
		fmt.Sprintf("Instance Number: %d", dicomImg.InstanceNumber),
		fmt.Sprintf("Modality: %s", dicomImg.Modality),
		fmt.Sprintf("Dimensions: %dx%d", dicomImg.Width, dicomImg.Height),
		fmt.Sprintf("Bits per Pixel: %d", dicomImg.BitsPerPixel),
